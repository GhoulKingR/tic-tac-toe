...
export default function Multi() {
  ...
  const [board, setBoard] = useState<BoardItem[]>(new Array(9).fill(BoardItem.None));
  ...
  function cpuMove() {
    setTimeout(() => {
      if (!win) {
        const choices = board.map((v, i) => [v, i]).filter(v => v[0] === BoardItem.None).map(v => v[1]);
        const randomIndex = Math.floor(Math.random() * choices.length);
        const randomChoice = choices[randomIndex];
        makeMove(randomChoice, true);
      }
    }, 2000);
  }
  ...
  function makeMove(pos: number, iscpu: boolean = false) {
    if (turn === player1 || iscpu) {
              ...
              >
                {winState.indexOf(key) !== -1 ? (
                  ...
                  item !== BoardItem.None ? (
                    ...
                    <Image
                      className={player1 === turn ? "only-hover" : "hover-hidden"}
                      src={player1 === BoardItem.X ? XOutlineIcon : OOutlineIcon}
                      alt="hover outline"
                    />
                  )
                )}
          ...
          >
            <section className="w-screen bg-semi-darknavy text-center pt-[40px] pb-[48px] md:pt-[45px] md:pb-[45px] font-outfit">
              <h1 className="font-bold text-[14px] md:text-[16px] tracking-[0.88px] text-silver mb-[16px]">
                {turn !== player1 ? "YOU WON!" : "OH NO, YOU LOST..."}
              </h1>
              <p
